...
# and `pip install ds_project` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "ds_project", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n ds_project -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "ds_project", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "ds_project", "YOUR_CONSOLE_SCRIPT"]
